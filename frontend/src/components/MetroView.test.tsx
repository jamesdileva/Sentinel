...
describe("MetroView", () => {
  it("renders one rail per visible tech line", () => {
    ...
    const { container } = render(<MetroView graph={graph} />);

    expect(rails(container)).toHaveLength(2);
    expect(container.querySelectorAll('g[data-kind="project"]')).toHaveLength(2);
  });

  it("line slider limits the rails and demotes projects to unserved chips", () => {
    const graph = makeGraph(
      [
        node("p1", "project", "Alpha"),
        node("p2", "project", "Beta"),
  ...
  it("a station appears on every line it uses at the same x (interchange)", () => {
    const graph = makeGraph(
      [
        node("p1", "project", "Alpha"),
        node("p2", "project", "Beta"),
      ...
      [link("p1", "react", "React"), link("p1", "rust", "Rust")],
    );
  ...
  it("projects never share an x slot on the same line", () => {
    const graph = makeGraph(
      [
        node("p1", "project", "Alpha"),
        node("p2", "project", "Beta"),
      ...
      [link("p1", "rust", "Rust"), link("p2", "rust", "Rust")],
    );
  ...
  it("hovering a station dims unrelated lines and stations", () => {
    const graph = makeGraph(
      [
        node("p1", "project", "Alpha"),
        node("p2", "project", "Beta"),
  ...
  it("clicking a station opens the focus panel; clicking again clears it", () => {
    const graph = makeGraph(
      [
  ...
  it("clicking a line reverse-focuses the stations on it", () => {
    const graph = makeGraph(
      [
        node("p1", "project", "Alpha"),
        node("p2", "project", "Beta"),
      ...
      [link("p1", "react", "React"), link("p2", "flask", "Flask")],
    );
  ...
  it("dragging slides a station and Reset restores it", () => {
    const graph = makeGraph(
      [
        node("p1", "project", "Alpha"),
        node("p2", "project", "Beta"),
  ...
  it("keeps checkout-dir detail in station tooltips for duplicate names", () => {
    const graph = makeGraph(
      [
      ...
      [link("a", "flask", "Flask"), link("b", "flask", "Flask")],
    );
  ...
  it("shows the tech summary sorted by usage", () => {
    const graph = makeGraph(
      [
        node("p1", "project", "Alpha"),
        node("p2", "project", "Beta"),
