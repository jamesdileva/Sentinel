...
const STATUS_COLOR: Record<string, string> = {
  ...
  findings: "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300",
};
...
export default function HealthCard({
  ...
  const chips: [string, string][] = [
    ...
    ["Docs", score.documentation_pct > 0 ? `${score.documentation_pct}%` : "none"],
  ];
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`text-3xl font-bold ${scoreColor(score.portfolio_score)}`}>
