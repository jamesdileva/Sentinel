...
/**
 * Read-only home-server status: Ollama availability/throughput + startup
 * checks. Shared by the Dashboard and the Settings page (v1.17.3 — moved
 * out of the System page so the dashboard shows it without a tab switch).
 * Per Project Rule 2 it never toggles anything server-side.
 */
export default function ServerStatus() {
  ...
  return (
    <>
      {error && (
        <div className="rounded-lg border border-red-300 bg-red-50 px-4 py-2 text-sm text-red-800 dark:border-red-800 dark:bg-red-950 dark:text-red-200">
          {error}
        </div>
      )}
...
export function OllamaPanel({ ollama }: { ollama: SystemOverview["ollama"] }) {
...
export function StartupPanel({
  ...
  states: SystemOverview["startup"]["states"];
}) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
