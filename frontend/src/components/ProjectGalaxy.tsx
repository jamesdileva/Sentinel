...
export default function ProjectGalaxy({ height = 480 }: { height?: number }) {
  ...
  if (error) return <p className="text-red-500 text-sm">Galaxy failed to load: {error}</p>
  if (!graph) return <p className="text-sm text-neutral-500">Loading galaxy…</p>

  const techIds = new Set(graph.nodes.filter((n) => n.kind === "tech").map((n) => n.id))
  const width = 800
    ...
    <div>
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full">
        {graph.links.map((link) => {
          ...
          if (!s || !t) return null
          return <line key={`${link.source}-${link.target}`} x1={s.x} y1={s.y} x2={t.x} y2={t.y} stroke="#a3a3a3" strokeWidth={1.5} />
        })}
        {graph.nodes.map((node) => {
          const pos = positions.get(node.id)
          if (!pos) return null
          const r = nodeSize(node.kind)
          return <circle key={node.id} cx={pos.x} cy={pos.y} r={r} fill="#1e1e1e" stroke={COLORS[node.id.length % COLORS.length]} strokeWidth={2} />
        })}
      </svg>
