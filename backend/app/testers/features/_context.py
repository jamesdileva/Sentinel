...
FEATURE_BUDGET_S = 120  # per-feature time budget
...
class FeatureContext:
    ...
    the tester session. `page` is the Playwright sync Page (msedge)."""

    def __init__(self, project, session_id, service, ctx: TesterContext, page):
        self.project = project
        self.session_id = session_id
        self.service = service
        self.ctx = ctx
        self.page = page
        self.deadline = time.monotonic() + FEATURE_BUDGET_S

    # ------------------------------------------------------------ recording

    def _check_budget(self) -> None:
        if time.monotonic() > self.deadline:
            raise TesterTimeoutError(f"Feature exceeded its {FEATURE_BUDGET_S}s budget")
    ...
    def go(self, url: str) -> None:
        ...
        move the page. Non-loopback hosts are refused before any request."""
