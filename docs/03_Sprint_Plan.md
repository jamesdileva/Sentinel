...
- `run.py` at the repo root is the single starting point: startup checks
  ...
  `--service` (skip when the port already serves), `--install`/`--uninstall`.
- `scripts/install_service.py` registers the `Sentinel` Task-Scheduler task:
  `pythonw run.py --service` every 5 minutes (idempotent, no admin needed).
