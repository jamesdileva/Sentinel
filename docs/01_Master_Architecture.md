...
- `appsession` / `sessioncheckpoint` / `sessionscreenshot` – session recorder
  rows (v1.17.10); screenshots live on disk at `/data/screenshots/<slug>/`
  (PNG + 90×60 thumb), the same `data/` root as the SQLite DB and
...
- **Always-on server**: Runs 24/7 (Task-Scheduler autostart), dashboard at `http://127.0.0.1:8420` (v1.17.8.1 — off 8000, the uvicorn default the indexed projects' dev servers use)
...
Desktop (dev machine + always-on server)
   │
   └── http://127.0.0.1:8420      Sentinel dashboard + API (native install)
