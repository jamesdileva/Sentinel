...
| 2026-08-20 | 1.17.18.0 | **Portfolio scoring fixes + tester-credit (docs/02 §14.5).** The portfolio's `screenshots_available` column was a hardcoded `False` stub (never read from `SessionScreenshot`) and its feature-matrix cell was hardcoded `✗`; now it counts any session screenshot for the project (join via the owning session) and contributes weight 5. Weights rebalanced from 30/30/25/15 to **build 30 / tests 30 / security 20 / docs 15 / screenshots 5** (=100). Tests evidence (v1.17.18.0) now also credits a **passed `Tester:` session** (the tester runner's own scripted runs are honest green evidence): precedence is latest-TestResult-red → `failing` (a failed reported run is never masked), green TestResult **or** passed tester session → `passing`; `_source_epoch` grew screenshot `captured_at` and tester-session `ended_at` so the change-driven cache recomputes when those appear. `refresh_all_scores()` runs at backend startup and recomputes every cached row — rows cached under older component definitions self-heal. New tests: +5 (tester credit, failed-tester no-credit, red-dominates-tester, screenshot component + epoch invalidation, refresh_all_scores) — 632 total. Live E2E 2026-08-20: `/api/v1/portfolio/scores` shows Ag/Airadio/FinSight/HFT/Cg/Card Game `screenshots_available=True`; Airadio/FinSight/HFT `test_status=passing` via tester credit; Ag stays `failing` (its red Tests-page row dominates). **Settings page (read-only + validation)**: new `GET /api/v1/settings` from `services/settings_service.py` (Rule 4) — every `SENTINEL_*` setting with value / default / source (env vs .env vs default, read from `os.environ` + `.env`), secrets (`SENTINEL_API_KEY`, `SENTINEL_GITHUB_TOKEN`) redacted to set/not-set; deterministic validation warnings reusing startup-check logic (watch-dir existence, chroma writability, port range) plus new checks (SQLite dir writability, 2s Ollama probe, embedding-model presence). Frontend: the System page (route `/system`, nav label "Settings") replaced by `pages/Settings.tsx` — grouped tables (Server / Paths / AI / Ops / World Sim) with source badges, warnings banner, home-server panel kept below. No write endpoints — editing config means editing `.env` and restarting (Rule 2). Tests: +6 backend (`test_settings.py` — groups/items, secret redaction + value never in response, env-source, port/ollama warnings, embedding-model warning), +5 frontend (groups/values/source badges, no-warnings banner, warnings with level badges, error state, ServerStatus retained) — 638 backend / 130 frontend. | |
| 2026-08-20 | 1.17.18.1 | **Adversarial audit fixes (docs/audit.md).** Five P1 findings shipped in-batch. A1: `scripts/backup.py` + `sentinel backup` CLI — SQLite online backup via `sqlite3.backup()` (WAL-safe) + chroma/screenshots/logs zipped into `data/backups/sentinel-<ts>.zip`, keeps newest N (default 7). A2: app-log rotation — `_rotate_app_log()` renames `<slug>.log` to `.log.1` when it exceeds `SENTINEL_APP_LOG_MAX_KB` (default 5120); `_log_lines` reads both files so markers survive rotation. A3: screenshot retention — `sweep_expired_screenshots()` at startup deletes rows + files older than `SENTINEL_SCREENSHOT_RETENTION_DAYS` (default 365; 0 = off). A4: process cleanup — `tester_runner.run()` wraps `service.end()` + `_kill_tree_best_effort` in `finally`; `command_runner` uses `Popen` + `CREATE_NEW_PROCESS_GROUP` + `taskkill /T /F /PID` on timeout. A5: dead `SENTINEL_API_KEY` removed. New config: `app_log_max_kb`, `screenshot_retention_days`. |
| 2026-08-20 | 1.17.17.0 | **Click-through chunk 2 (HFT input scripting, docs/clickthrough_plan.md) — shipped.** Ground truth re-verified 2026-08-20 (GUI.cpp): window "HFT Order Book" is SDL2 + OpenGL3 + Dear ImGui 1.92.6, `gui.init(1280, 760)` — resizable — with all random streams seeded 42 (mt19937_64) so runs render deterministically; PrintWindow(PW_RENDERFULLCONTENT\|PW_CLIENTONLY) renders the GL window (no screen-crop fallback). Buttons measured: BENCHMARK MODE (628,323) #143B73, TRADING GAME (628,412) #1A5926, MAIN MENU (1178,723), START TRADING (640,409); the picker's `TRADE THIS STOCK` button is the card child's last element — a full-card-width bar at the card bottom — located by its fill-color search in the card-1 bottom band (unpressed fill #21262E = ImGui FrameBg), not a fixed coordinate; live-verified gotcha: the button is NOT green (the earlier green search matched the COL_GREEN "Start price" text), and ImGui hover-tints buttons under the physical cursor (#1A5926 → #263880) so the engine gained `move_mouse` to park the cursor off controls. Benchmark (2M orders, ~1-2 s) does NOT auto-return — the feature clicks MAIN MENU; trading (100k @ 1× ≈ 40 s) auto-ends on a pixel-stable SessionEnd. New engine helpers in `desktop_runner.py`: `pin_window` (SetWindowPos move + client-size verify — SDL sizes are client-area, never forced; mismatch → TesterEnvError), `find_color_bbox` (sampled color search, min_pixels rejects stray antialiasing), `move_mouse` (SetCursorPos without click — hover-tint parking), `wait_region_stable` (streak of consecutive identical captures). New feature `app/testers/features/hft_order_book.py` (native, budget 240 s): taskkill-reclaim → launch `build\hft.exe` (the HFT presence tester stays presence-only, Rule 4) → attach ^HFT Order Book$ → bring to front → pin (40,40) verify 1280×760 → menu signature → benchmark (region change + settle) → MAIN MENU → TRADING GAME → find_color_bbox card-1 button → START TRADING → settle → SessionEnd, screenshots per stage, taskkill cleanup in finally. Registry gains Hft-Order-Book. Tests: +4 engine (pin move+flags, wrong-client env error, bbox locate/tolerance/sparse, stable return/timeout), +2 feature (fake-desktop happy path + honest failure on missing card button), registry test updated — 618 total, coverage 90.15%, lint clean. **Live E2E passed 2026-08-20 01:59–02:00** (all 19 steps, 6 stage screenshots). | |
| 2026-08-19 | 1.17.16.0 | **Click-through Phase 3 chunk 1: native desktop engine (pywinauto) + AG features (docs/clickthrough_plan.md).** The plan's 2026-08-18 ground truth was WRONG on this machine — live-verified: Tcl/Tk 8.6.15 ships no MSAA/UIA tree (`tk::msaa` absent), so pywinauto element-driving of tkinter widgets is impossible; tk also ignores posted WM_* mouse messages (0 changed pixels); capture is only reliable via `PrintWindow(PW_RENDERFULLCONTENT\|PW_CLIENTONLY)` through ctypes (pywinauto's own capture is garbage on these windows, ImageGrab is occlusion-bound); `SetWindowPos` HWND_TOPMOST fails (ERROR_INVALID_WINDOW_HANDLE 1400) and SetForegroundWindow cannot override an app holding foreground rights (MapleStory while gaming) — so the engine raises an honest, retryable TesterEnvError ("desktop busy"). New `app/services/desktop_runner.py` (`DesktopApp`): UIA attach by title pattern (Rule-1 guard: never drive an arbitrary window), bring_to_front (SW_RESTORE + AttachThreadInput dance + BringWindowToTop + SetForegroundWindow verify loop), capture via PrintWindow flags 3, `click(x,y)` = SetCursorPos + physical SendInput (absolute), native dialogs by element name (`dialog(title_re)` / `element`), `changed_pixels` (grid step 4) / `wait_region_change` / `assert_pixel` (tolerance 6). `Feature.native=True`; `FeatureContext` gains native branch (shot → `ctx.desktop.shot`, go() refused); `FeatureRunner.run` partitions native vs web. AG feature: attach `^AG Character & Weapon Studio$` → bring to front → layout signature via measured anchors (tab accent (10,20)=(91,141,239), bottom-bar fill (25,510)=(35,38,46), status bg (100,560)=(21,23,28); window moved to a fixed position first, clicks from the live rect) → real pose via the native `Select T-Pose Image` dialog (`File name:` set_text + `Open`) → `Generate Character` → **transition only** (status + entry region change; SF3D takes 5–10 min, completion NOT asserted — Rule 3, same as Cg RESEARCHING; budget_s 600). AG tester docstring updated: the animate path is green since AG fixed the root_motion NameError (`bb27c7b`). pywinauto added to `pyproject.toml`. Tests: +11 — 8 DesktopApp unit tests (title guard, connect attach, missing-window env error, foreground grant, busy-desktop env error, region diff, wait timeout, assert tolerance, budget) + 3 native feature tests (fake-desktop E2E, busy→investigate mapping, native go-refused); registry gains Ag; all-features fake-page test now covers web features only (native constructs its own engine — covered by the fake-desktop tests). Gate: pytest + black/isort/flake8 green; live E2E recorded on this machine. | |
