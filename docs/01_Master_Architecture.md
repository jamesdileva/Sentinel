...
|------|---------|--------|--------|
| 2026-08-06 | 1.11 | Sprint 11 (Testing & QA): quality gate established — 211 backend pytest (95.6% coverage, ≥ 80% gate), 29 Vitest component tests (jsdom + jest-dom), 7 Playwright E2E specs against the real stack (backend uvicorn + Vite dev, auto-scan disabled for determinism). Frontend api client timeout raised 10s→30s (portfolio recompute). Details in impl guide §12, sprint plan v1.11 | User + AI agent |
