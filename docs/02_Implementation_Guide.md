...
|------|---------|--------|--------|
| 2026-08-10 | 1.17.3 | Living-week fixes from the first index run. **Embedding t/s fixed**: real Ollama never returns `prompt_eval_duration` from `/api/embed` (verified live against 0.32.6 — only `prompt_eval_count`/`total_duration`/`load_duration` come back), so index ticks silently carried no speed; `embed_with_metrics` now falls back to `total_duration − load_duration` and the feed shows "~N tok/s" again. **Activity history ceiling repaired**: the prune DELETE hard-coded `activity_event`, but SQLModel names the table `activityevent` — it threw on *every* publish (warning spam since v1.17, 5000-row ceiling never ran); the DELETE now uses `ActivityEvent.__tablename__`. **Repo sync actually works now**: `run_command` passed `env=` to subprocess, which *replaces* the whole environment — no PATH, so every git invocation failed with "'git' is not recognized" (all 18 laptop clones/pulls); env now overlays the inherited environment, and a new `resolve_git()` (`SENTINEL_GIT_EXECUTABLE` → `shutil.which` → Git-for-Windows paths, quoted when embedded in commands) covers the Task Scheduler's minimal PATH with a clear "git not found" sync error. **System → Dashboard merge**: Ollama + startup-check panels moved to the Dashboard ("Home server" section, shared `components/ServerStatus.tsx`); `/system` stays as the future Settings page (sync interval, watch dirs, auto token retrieval — deferred, docs/later.md). The "sync skipped — token not configured" pill was stale history from pre-token boots (broken prune kept it) — self-corrects once sync succeeds. Tests: regression tests for all four fixes, 296 backend (94.2%) / 71 vitest | AI agent |
