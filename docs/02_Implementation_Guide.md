...
| 2026-08-16 | 1.17.12.3 | **Window-targeted screenshots: app windows instead of the whole desktop.** Screenshot capture (manual Capture button, tester ctx.screenshot, auto end-of-session) now finds the app under test's own top-level window and crops the grab to it. New module app/utils/window_capture.py (stdlib ctypes only): EnumWindows + GetWindowThreadProcessId + QueryFullProcessImageNameW matching the window process's executable path or a bounded ancestor chain (casefold prefix; largest visible, non-minimized, non-zero-area window wins; never title matching). capture() in app_sessions.py crops ImageGrab.grab(bbox=...) to the rect, clamped to the virtual screen. Headless apps (WFT, Demake), closed/minimized windows fall back to the full-screen grab unchanged. Honest limitation documented: a cropped screen grab captures whatever is stacked above an occluded window (PrintWindow rejected - black frames for GPU-rendered Electron windows). Live bug: AG's tkinter GUI is re-executed by its venv python into the base interpreter, so the window's own exe lives under AppData \u2014 the match walks up to 6 ancestors (Toolhelp snapshot) and still finds the venv python under the project dir (regression-tested). Tests: +6 backend (bbox passed, full-screen fallback, clamp math, ancestor-chain match, re-executed ancestor, bounded depth); 503 backend / 128 frontend green. | AI agent |
